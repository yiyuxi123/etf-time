/**
 * Standalone Dashboard Data Service
 * Ported from api/dashboard.ts — runs entirely in the browser/Capacitor WebView.
 * Replaces yahoo-finance2 Node.js package with direct Yahoo Finance REST API calls.
 */
import type { DashboardData, MarketData, ChartDataPoint, EtfInfo, FactorBreakdown } from '../types';
...
const calculateRSI = (history: any[], period = 14) => {
  let valid = history.filter((h: any) => h && h.close != null);
...
// ============================================================================
// SCORING ENGINES (identical to server logic)
// ============================================================================

function calculateSwingScore(
  id: string, price: number, ma200: number, ma50: number, ma50Up: boolean,
  rsi: number, drawdown: number, pe: number, vix: number, volatility: number,
  usdcny: number, tnx: number
) {
  ...
  if (id === 'ndx') {
    let weight200MA = 25, weight50MA = 15, weightPE = 15, weightVIX = 10;
    const weightUSDCNY = 15;
    if (vix > 25) { weightPE = 22; weight50MA = 8; }
    else if (vix < 15) { weight50MA = 20; weightVIX = 5; }
    ...
    const weight200MA = 25, weight50MA = 15, weightPE = 15, weightRSI = 15, weightSinoUS = 10;
    const ma200Score = price > ma200 ? weight200MA : 0;
    const ma50Score = (price > ma50 && ma50Up) ? weight50MA : (price > ma50 ? weight50MA * 0.6 : 0);
    const peScore = clamp((14 - pe) / 4 * weightPE, 0, weightPE);
    let rsiScore = 10;
    if (rsi >= 45 && rsi <= 65) rsiScore = weightRSI;
    else if (rsi > 65) rsiScore = clamp(weightRSI - (rsi - 65) / 10 * 10, 5, weightRSI);
    else rsiScore = clamp((rsi - 30) / 15 * weightRSI, 0, weightRSI);
    ...
    let rsiScore = 10;
    if (rsi >= 45 && rsi <= 65) rsiScore = 20;
    else if (rsi > 65) rsiScore = clamp(20 - (rsi - 65) / 15 * 15, 5, 20);
    else rsiScore = clamp((rsi - 35) / 10 * 20, 0, 20);
...
function calculateDCAScore(
  id: string, price: number, ma200: number, ma50: number,
  rsi: number, drawdown: number, pe: number, vix: number, volatility: number,
  usdcny: number, tnx: number
) {
  price = typeof price === 'number' && !isNaN(price) ? price : 1;
  ...
  if (id === 'ndx') {
    let weightPE = 25, weightVIX = 15;
    const weight200MA = 15, weightDrawdown = 15, weightExchange = 10;
    if (vix > 25) { weightPE = 30; weightVIX = 10; }
    const peScore = clamp((35 - pe) / 10 * weightPE, 0, weightPE);
    const vixScore = clamp((vix - 15) / 15 * weightVIX, 0, weightVIX);
    const devPct = ((price - ma200) / ma200) * 100;
    ...
    const weightPE = 25, weight200MA = 15, weightRSI = 15, weightDrawdown = 15, weightSinoUS = 10;
    const peScore = clamp((14 - pe) / 4 * weightPE, 0, weightPE);
    const devPct = ((price - ma200) / ma200) * 100;
    const ma200Score = price <= ma200 ? weight200MA : clamp(weight200MA - (devPct / 15) * weight200MA, 0, weight200MA);
...
async function fetchMarketData(
  ...
  // Fetch benchmark quote and chart
  const [benchmarkQuote, benchmarkHistory] = await Promise.all([
    getQuote(benchmarkSymbol),
    getChart(benchmarkSymbol, period1),
  ]);
  ...
  // Fetch VIX if needed
  let vixValue = 15;
  let vixHistoryData: any[] = [];
  if (useVix) {
    try {
      const [vq, vh] = await Promise.all([
        getQuote('^VIX'),
        getChart('^VIX', period1),
      ]);
      vixValue = vq?.regularMarketPrice || 15;
      vixHistoryData = vh?.quotes || [];
    } catch (e) {
      vixValue = 15;
    }
  }
  ...
  if (useVix) {
    vixHistoryData.forEach((q: any) => {
      if (q.date && q.close !== null && q.close !== undefined && !isNaN(q.close)) {
        vixMap.set(new Date(q.date).toISOString().split('T')[0], q.close);
      }
    });
  }
  ...
  const historyLengths = benchmarkQuotes.length;
  ...
  for (let i = 0; i < historyLengths; i++) {
    const dateStr = new Date(benchmarkQuotes[i].date).toISOString().split('T')[0];
    const close = benchmarkQuotes[i].close;
    if (!close) continue;
    closes.push(close);

    if (closes.length > ma200WindowSize) closes.shift();

    const ma200 = closes.reduce((a: number, b: number) => a + b, 0) / closes.length;
    ...
    if (i >= 14) {
      const histSlice = benchmarkQuotes.slice(0, i + 1);
    ...
    const ma50 = recent50.reduce((a: number, b: number) => a + b, 0) / Math.min(closes.length, 50);
    const recent50Prev = closes.slice(-51, -1);
    const ma50Prev = recent50Prev.length > 0 ? (recent50Prev.reduce((a: number, b: number) => a + b, 0) / Math.min(closes.length - 1, 50)) : ma50;
    ...
    if (i >= 20) {
      ...
      const variance = recent.reduce((a: number, b: number) => a + Math.pow(b - mean, 2), 0) / recent.length;
  ...
  const ma200 = benchmarkQuote.twoHundredDayAverage || (closes.length > 0 ? closes.reduce((a: number, b: number) => a + b, 0) / closes.length : price);
  const ma50 = benchmarkQuote.fiftyDayAverage || (closes.length > 0 ? closes.slice(-50).reduce((a: number, b: number) => a + b, 0) / Math.min(closes.length, 50) : price);
  ...
  // ETF data from Tencent
  let etfResults: EtfInfo[] = [];
  if (tencentEtfs.length > 0) {
    try {
      const tencentRes = await httpGet(`https://qt.gtimg.cn/q=${tencentEtfs.map(e => e.symbol).join(',')}`);
      const tencentText = tencentRes;
      ...
      etfResults = tencentEtfs.map((etf) => {
        ...
        } else if (swingTotalScore >= 70) swingRecommendation = '强烈买入 (右侧确立买入)';
        else if (swingTotalScore >= 50) swingRecommendation = '买入 (突破加仓)';
        else if (swingTotalScore >= 30) swingRecommendation = '持有 (正常回踩观察)';
        else if (swingTotalScore >= 15) swingRecommendation = '减仓 (破位止损观望)';
        else swingRecommendation = '清仓 (超买分批止盈)';

        // DCA Premium Score
        let dcaPremiumScore = 0;
        if (premiumPct <= 0) dcaPremiumScore = 20;
        else if (premiumPct > 0 && premiumPct <= 0.5) dcaPremiumScore = 20 - premiumPct * 20;
        else dcaPremiumScore = Math.max(-20, 10 - (premiumPct - 0.5) * 40);
        ...
        } else if (dcaTotalScore >= 70) dcaRecommendation = '强烈买入 (黄金吸筹窗口)';
        else if (dcaTotalScore >= 50) dcaRecommendation = '买入 (分批低吸/常规定投)';
        else if (dcaTotalScore >= 30) dcaRecommendation = '持有 (高位暂停扣款)';
        else if (dcaTotalScore >= 15) dcaRecommendation = '减仓 (红盘控仓/暂停扣款)';
        else dcaRecommendation = '清仓 (减少扣款/暂停定投观望)';
      ...
      console.error(`Tencent fetch failed for ${id}`, e);
    }
  }

  // OTC funds from Eastmoney
  if (otcFunds.length > 0) {
    for (const otc of otcFunds) {
      try {
        const code = otc.symbol.replace('f_', '');
        const res = await httpGet(`http://fundgz.1234567.com.cn/js/${code}.js`);
        const jsonStr = res.match(/jsonpgz\((.*)\);/);
        if (jsonStr && jsonStr[1]) {
          ...
          let swingRecommendation = '';
          if (swingTotalScore >= 70) swingRecommendation = '强烈买入 (右侧确立买入)';
          else if (swingTotalScore >= 50) swingRecommendation = '买入 (突破加仓)';
          else if (swingTotalScore >= 30) swingRecommendation = '持有 (正常回踩观察)';
          else if (swingTotalScore >= 15) swingRecommendation = '减仓 (破位止损观望)';
          else swingRecommendation = '清仓 (超买分批止盈)';
          ...
          let dcaRecommendation = '';
          if (dcaTotalScore >= 70) dcaRecommendation = '强烈买入 (黄金吸筹窗口)';
          else if (dcaTotalScore >= 50) dcaRecommendation = '买入 (分批低吸/常规定投)';
          else if (dcaTotalScore >= 30) dcaRecommendation = '持有 (高位暂停扣款)';
          else if (dcaTotalScore >= 15) dcaRecommendation = '减仓 (红盘控仓/暂停扣款)';
          else dcaRecommendation = '清仓 (减少扣款/暂停定投观望)';
  ...
  return {
    id,
    name,
    quote: { price, changePct: benchmarkQuote.regularMarketChangePercent || 0 },
...
export async function fetchDashboardData(customMarkets?: any[]): Promise<DashboardData> {
  console.log('[Standalone Dashboard] Fetching global macro indicators...');
  const period1 = new Date();
  period1.setFullYear(period1.getFullYear() - 2);

  // Macro data
  const [usdcnyQuote, usdcnyChart, tnxQuote, tnxChart] = await Promise.all([
    getQuote('CNY=X').catch(() => ({ regularMarketPrice: 7.23 })),
    getChart('CNY=X', period1).catch(() => ({ quotes: [] })),
    getQuote('^TNX').catch(() => ({ regularMarketPrice: 4.25 })),
    getChart('^TNX', period1).catch(() => ({ quotes: [] })),
  ]);
  ...
  (usdcnyChart?.quotes || []).forEach((q: any) => {
    if (q.date && q.close !== null && q.close !== undefined && !isNaN(q.close)) {
  ...
  (tnxChart?.quotes || []).forEach((q: any) => {
    if (q.date && q.close !== null && q.close !== undefined && !isNaN(q.close)) {
      tnxMap.set(new Date(q.date).toISOString().split('T')[0], q.close);
    }
  });

  console.log(`USD/CNY: ${usdcnyPrice}, TNX: ${tnxPrice}. Fetching market data...`);

  const userMarkets = customMarkets;

  const markets = await Promise.all([
    fetchMarketData('ndx', '纳斯达克100', 'QQQ',
      userMarkets?.find((m: any) => m.id === 'ndx')?.etfSymbols || [
      ...
      ], true, usdcnyPrice, usdcnyMap, tnxPrice, tnxMap),
    fetchMarketData('csi300', '沪深300', '000300.SS',
      userMarkets?.find((m: any) => m.id === 'csi300')?.etfSymbols || [
      ...
      ], false, usdcnyPrice, usdcnyMap, tnxPrice, tnxMap),
    fetchMarketData('gold', '黄金', 'GC=F',
      userMarkets?.find((m: any) => m.id === 'gold')?.etfSymbols || [
    ...
    fetchMarketData('bond', '国内债市', '511010.SS',
      userMarkets?.find((m: any) => m.id === 'bond')?.etfSymbols || [
...
export async function getDashboard(customMarkets?: any[]): Promise<DashboardData> {
  const now = Date.now();
  const isCustom = !!customMarkets;

  if (!isCustom && dashboardCache.data && now - dashboardCache.timestamp < CACHE_TTL) {
    return dashboardCache.data;
  }

  const data = await fetchDashboardData(customMarkets);
