...
async function startServer() {
  ...
  app.get("/api/dashboard", async (req, res) => {
    try {
      ...
      // Tencent API uses GBK but numbers are ASCII so text is fine
      ...
      const etfResults: EtfInfo[] = targetETFs.map((etf) => {
        ...
        // Fallback calculation in case API structure completely flips
        ...
        let premiumScore = 0;
        if (premiumPct <= 0) premiumScore = 20; // 折价满分 20
        ...
        return {
          symbol: etf.symbol.replace('sh', '').replace('sz', ''), // Clean up for UI display
          name: etf.name,
          price,
          estimatedIopv,
          premiumPct,
      ...
      let sumClose = 0;
      let windowSize = 50; // Use 50MA for pseudo "trend" history since 200 is too long for 1 year chart
      let closes: number[] = [];
      ...
      for (let i = 0; i < historyLengths; i++) {
        ...
        // Mock PE assuming identical trailing EPS over the short window
        const estimatedHistoricalPe = qqqPE * (close / qqqPrice);
        
        // Mock Trend difference vs MA
        const trendPct = ((close - ma) / ma) * 100;

        chartData.push({
          date: dateStr,
          close: close,
          vix: vixMap.get(dateStr) || vixValue, // fallback to current if gap
